...
Failed runs can be continued with `--resume` when their Git commit, working-tree state, mode, and profile still match; this reuses Pi's native session for legacy single-pass runs and reuses completed partial reports for scoped runs. Four explicit bundled profiles are supported: generic, IRC bot, web application, and simulation. Common language/tooling markers are detected, and JavaScript syntax can be checked with Node's non-executing `--check` mode. Bounded line-range reads and a configurable tool-call ceiling prevent reviews from searching indefinitely; context-limit failures are classified explicitly. Large repositories are divided into file-level scopes bounded by source size and tool budget, reviewed in fresh restricted sessions, given one bounded follow-up pass for explicitly unreviewed paths, and recursively synthesized in fresh sessions so neither investigation nor synthesis depends on one giant context. A strict repository-local `.nightwatch/config.json` supplies stable non-secret CLI defaults without another parser dependency; CLI values take precedence. Broader analyzers that may load repository configuration or execute project code, global configuration, project-defined profiles, custom checkpoint/compaction/event logs, and council mode are deferred. The Pi SDK remains uninstalled unless executable integration exposes a concrete limitation.
